...
class EDA:
    ...
    def refined_clean_up(self):
        ...
        self.acc_df['month'] = self['issue_d'].apply(lambda x: x.month)
        self.acc_df['year'] = self.acc_df['issue_d'].apply(lambda x: x.year)
    ...
    def bar(self, x, figsize=(10,5)):
        plt.figure(figsize=figsize)
        sns.barplot(x=x, y='loan_status', data=self.acc_df)
